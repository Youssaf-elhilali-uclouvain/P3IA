...
class SmartStrategy(AntStrategy):
    ...
    def decide_action(self, perception: AntPerception) -> AntAction:
        ...
        mem["deposit_counter"] = (mem["deposit_counter"] + 1) % 4
        if mem["deposit_counter"] == 0:
            if perception.has_food:
                mem["last_action"] = AntAction.DEPOSIT_FOOD_PHEROMONE
                mem["last_direction"] = None
                return AntAction.DEPOSIT_FOOD_PHEROMONE
            else:
                mem["last_action"] = AntAction.DEPOSIT_HOME_PHEROMONE
                mem["last_direction"] = None
    ...
    def _get_memory(self, ant_id: int) -> dict:
        if ant_id not in self.memory:
            self.memory[ant_id] = {
                "visited": set(), "colony_pos": None, "food_targets": [], "abs_x": 0, "abs_y": 0, "last_action": None, "last_direction": None,
                "last_move_blocked": False, "stuck_count": 0, "explore_bias": random.choice([-1, 1]), "path_history": [], "returning": False,
                "wander_steps": 0,
                "deposit_counter": 0,
            }
    ...
    def _return_to_colony(self, mem: dict, perception: AntPerception) -> AntAction:
        if perception.can_see_colony():
            target_DVal = perception.get_colony_direction()
            if target_DVal is not None:
                return self._steer_toward(target_DVal, perception, mem)
        ...
        if mem["colony_pos"] is not None:
            ...
            target_DVal = self._delta_to_direction(dx, dy)
            return self._steer_toward(target_DVal, perception, mem)
    ...
    def _seek_food(self, mem: dict, perception: AntPerception) -> AntAction:
        if perception.can_see_food():
            mem["wander_steps"] = 0
            target_DVal = perception.get_food_direction()
            if target_DVal is not None:
                return self._steer_toward(target_DVal, perception, mem)
        ...
        if perception.food_pheromone:
            best = max(perception.food_pheromone, key=perception.food_pheromone.get)
            if perception.food_pheromone[best] > 0:
                target_DVal = self._delta_to_direction(best[0], best[1])
                return self._steer_toward(target_DVal, perception, mem)
    ...
    def _explore(self, mem: dict, perception: AntPerception) -> AntAction:
        ...
        candidates = [(AntAction.MOVE_FORWARD, current_dir), (AntAction.TURN_LEFT,    Direction.get_left(current_dir)),(AntAction.TURN_RIGHT,  Direction.get_right(current_dir)),]
    ...
    def _steer_toward(self, target_DVal: int, perception: AntPerception, mem: dict) -> AntAction:
        current_val = perception.direction.value
        diff = (target_DVal - current_val) % 8
